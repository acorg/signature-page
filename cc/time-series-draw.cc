...
void TimeSeriesDraw::prepare()
{
    ...
    if (!sequences_per_month.empty()) {
        ...
        std::cout << "INFO: dates to show: " << mSettings.begin << " .. " << mSettings.end << "  months: " << mNumberOfMonths << DEBUG_FILE_LINE_FUNC << '\n';
    }
    else {
        std::cout << "WARNING: no dates found for sequences" << DEBUG_FILE_LINE_FUNC << '\n';
        mNumberOfMonths = 0;
    }
