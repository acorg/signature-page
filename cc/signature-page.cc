...
#include "signature-page/settings.hh"
#include "signature-page/mapped-antigens-draw.hh"
#include "signature-page/antigenic-maps-draw.hh"
#include "signature-page/title-draw.hh"
...
void SignaturePageDraw::make_surface(std::string_view aFilename, bool init_settings, bool show_aa_at_pos, bool draw_map)
{
    ...
    if (!mChartFilename.empty()) {
        mAntigenicMapsDraw = std::unique_ptr<AntigenicMapsDrawBase>(make_antigenic_maps_draw(mChartFilename, mSurface->subsurface(false), *mTree, *mSettings->hz_sections, *mSettings->signature_page, *mTimeSeriesDraw, *mSettings->antigenic_maps));
...
void SignaturePageDraw::make_layout_tree_ts_clades()
{
    ...
    if (mSettings->aa_at_pos->width > 0.0)
        mAAAtPosDraw->surface().move_resize_viewport(acmacs::PointCoordinates(aa_at_pos_left, mSettings->signature_page->top), mSettings->aa_at_pos->width,
