...
void TreeDraw::draw_node(const Node& aNode, double aOriginX, double& aVerticalGap, double aEdgeLength)
{
    if (aNode.draw.shown) {
        ...
        if (aNode.is_leaf()) {
            ...
            const acmacs::Location2D text_origin{right + mNameOffset, aNode.draw.vertical_pos + tsize.height / 2};
...
void TreeDraw::draw_aa_transition(const Node& aNode, acmacs::Location2D aOrigin, double aRight)
{
    ...
    if (settings->show && !aNode.data.aa_transitions.empty() && aNode.data.number_strains >= settings->number_strains_threshold) {
        if (auto labels = aNode.data.aa_transitions.make_labels(settings->show_empty_left); !labels.empty()) {
            ...
            if (branch_settings.show) {
                ...
                acmacs::Location2D origin = aOrigin + offset;
                ...
                const acmacs::Location2D connection_line_start{(aOrigin.x() + aRight) / 2, aOrigin.y()};
                acmacs::Location2D connection_line_end;
                const acmacs::Offset label_offset = branch_settings.label_offset;
                if (label_offset.y() > 5)
                    connection_line_end = acmacs::Location2D{aOrigin.x() + longest_label_size.width / 2 + offset.x(), aOrigin.y() - longest_label_size.height + offset.y()};
                else if (label_offset.y() < -5)
                    connection_line_end = acmacs::Location2D{aOrigin.x() + longest_label_size.width / 2 + offset.x(), aOrigin.y() + offset.y()};
                else if (label_offset.x() > 0)
                    connection_line_end = acmacs::Location2D{aOrigin.x() + offset.x(), aOrigin.y() + offset.y() - longest_label_size.height / 2};
                else
                    connection_line_end = acmacs::Location2D{aOrigin.x() + longest_label_size.width + offset.x(), aOrigin.y() + offset.y() - longest_label_size.height / 2};
...
{
    if (auto found = sections.find_if([&seq_id](const auto& sect) { return HzSection(sect).name == seq_id; }); !found) {
          // std::cerr << "DEBUG: add hz section: " << seq_id << '\n';
        auto sec = sections.emplace_back();
        sec.name = seq_id;
        sec.show_line = show_line;
        if (!clade.empty())
            sec.triggering_clades.push_back(clade);
        if (aa_pos > 0)
            sec.triggering_aa_pos.push_back(aa_pos);
    }
    else {
        if (!clade.empty())
...
void HzSections::add(const Node& node, bool show_line, std::string clade, size_t aa_pos)
{
    add(node.seq_id, show_line, clade, aa_pos);
...
#pragma GCC diagnostic pop
    ...
    auto match_entry = [label = aLabels.label(), aFirstLeafSeqid](const auto& entry_v) {
                           const AATransitionIndividualSettings entry(entry_v);
    ...
    if (scatter_label_offset > 0.0) {
        ...
        settings["label_offset"] = rjson::array{static_cast<double>(rand()) * scatter_label_offset * 2 / rand_size - scatter_label_offset + old_label_offset.x(),
