...
void TreeDraw::draw()
{
    ...
    mLineWidth = Scaled{mSettings.force_line_width ? line_width : std::min(line_width, mVerticalStep * 0.5)};
...
{
    mFontSize = Scaled{mVerticalStep};
                ...
                if (clade.second.shown()) {
                    for (const auto& sect : clade.second.sections) {
                        if (clade.first != "GLY" && clade.first != "NO-GLY") {
                            // std::cerr << "DEBUG: clade: " << clade.first << ' ' << s << '\n';
